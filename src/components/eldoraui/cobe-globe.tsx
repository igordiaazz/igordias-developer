...
export function Cobe({
  ...
  const targetDragOffset = useRef(0)
  ...
  useEffect(() => {
    ...
    const render = () => {
      const w = width * 2
      const h = variant === "scaled" ? width * 2 * 0.4 : width * 2

      // Smooth the drag offset (replaces react-spring)
      dragOffset.current += (targetDragOffset.current - dragOffset.current) * 0.1

      switch (variant) {
        case "default":
          phi += 0.005
          globe.update({ phi: phi + dragOffset.current, width: w, height: h })
          break
        case "draggable":
          globe.update({ phi: dragOffset.current, width: w, height: h })
          break
        case "auto-draggable":
        ...
        case "auto-rotation":
          phi += 0.005
          globe.update({ phi, width: w, height: h })
          break
        case "rotate-to-location": {
          ...
          globe.update({ phi: currentPhi, theta: currentTheta, width: w, height: h })
          break
        }
        case "scaled":
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    if (
      ...
      targetDragOffset.current = delta / 200
    }
  }
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    if (
      ...
      pointerInteractionMovement.current = delta
      targetDragOffset.current = delta / 100
    }
  }
